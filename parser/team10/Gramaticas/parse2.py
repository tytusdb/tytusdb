...
#Operaciones Extras
#adicion de produccionde funciones matematicas
...
def p_math_funciones(t):
        ...
        node= grammer.nodoDireccion('math_functions')
        node.agregar(t[1])
        node.agregar(t[3])
        t[0]= node
...
def p_funciones_math1(t):
        ...
        | radians
        | sign'''
        node= grammer.nodoDireccion('funciones_math1')
        node1= grammer.nodoDireccion(t[1])
        node.agregar(node1)
        t[0]= node
...
def p_funciones_math2(t):
        ...
        | mod
        | round'''
        node= grammer.nodoDireccion('funciones_math2')
        node1= grammer.nodoDireccion(t[1])
        node.agregar(node1)
        t[0]= node
...
def p_trigonometricas(t):
        ...
        node= grammer.nodoDireccion('f_trigonometricas')
        node.agregar(t[1])
        node.agregar(t[3])
        t[0]= node
...
def parse(input):
        ...
        arbolimas = generar.graficaArbol(parsear)
        arbolimas.ejecutarGrafica()

# f = open("D:/CURSOS 2DO SEMESTRE 2020/VACACIONES/COMPILADORES/APOYO/COMPI2_DIC2020/Gramaticas/entrada.txt", "r")
# input = f.read()
# print(input)
