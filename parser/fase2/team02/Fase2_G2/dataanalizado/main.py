...
class interfaz():
    def __init__(self):
        ...
        self.window.title("Reporte traducido")
        ...
        lblentrada= Label(self.window,text="Archivo de Entrada:",height=1, width=15,bg='#2747A0')
        lblentrada.place(x=20,y=80)
        lblsalida= Label(self.window,text="Consola de Salida:",height=1, width=15,bg='#2747A0')
        ...
        self.file=""
        self.abrirandcolocar()

        self.window.mainloop()

    def abrirandcolocar(self):
        print("abrirandcolocar")

        try:
            script_dir = os.path.dirname(__file__) #<-- absolute dir the script is in
            f = open(script_dir+"/traducido0.txt", "r")     

            # f = open("C:/Users/PC_CASA/Documents/CLASE/PROYECTO2/Tytus_SQLPARSER_G8/dataanalizado/traducido0.txt", "r")     
            entrada1=f.read()
            f.close()

            entrada="CREATE DATABASE IF NOT EXISTS test    OWNER = 'root'    MODE = 1;USE test; "
            entrada+=entrada1
            self.crear_tab(entrada,"traducido")
            self.btnanalizar_click()


            # self.lizar_click(entrada)
            print("He!")
    ...
    def crear_tab(self,entrada,nombre):
        self.tab_frame.append(Frame(self.tab,width=200, height=700,background="#2747A0"))
