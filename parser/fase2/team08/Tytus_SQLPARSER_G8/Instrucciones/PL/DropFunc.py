...
class DropFunc(Instruccion):
        ...
        if existe != None:
            if self.parametros == None:
                ...
                if (listaParametros == listaArgumentos):
                    ...
                    error = Excepcion("42723", "Semantico", f"La función: {self.id} no existe. Los tipos de parámetros no coinciden.", self.linea, self.columna)
                    arbol.excepciones.append(error)
                    arbol.consola.append(error.toString())
                    return error
        else:
    ...
    def traducir(self, tabla, arbol):
        ...
        if not self.ignora:
            arbol.addComen(f"Se elimina la función -> {self.id}")
            arbol.addc3d(f"del globals()[\"{self.id}\"]")
            arbol.addc3d(f"print(\"La función: {self.id} ha sido eliminada con éxito\")")