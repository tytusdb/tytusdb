...
class Cosd(Instruccion):
    ...
    def ejecutar(self, tabla, arbol):
        ...
        if self.valor.tipo.tipo != Tipo_Dato.SMALLINT and self.valor.tipo.tipo != Tipo_Dato.INTEGER and self.valor.tipo.tipo != Tipo_Dato.BIGINT and self.valor.tipo.tipo != Tipo_Dato.DECIMAL and self.valor.tipo.tipo != Tipo_Dato.NUMERIC and self.valor.tipo.tipo != Tipo_Dato.REAL and self.valor.tipo.tipo != Tipo_Dato.DOUBLE_PRECISION:
            error = Excepcion('42883',"Semántico","No existe la función cosd("+self.valor.tipo.toString()+")",self.linea,self.columna)
            arbol.excepciones.append(error)
            arbol.consola.append(error.toString())
            return error
        try:
            if resultado % 90 == 0:
                return (1, 0, -1, 0)[int(resultado // 90) % 4]
            else:
                return math.cos(math.radians(resultado))
    ...
    def traducir(self, tabla, arbol):
        ...
        if isinstance(self.valor, Primitivo):
            return f"COSD({self.valor.traducir(tabla,arbol).temporalAnterior})"
        elif isinstance(self.valor, Aritmetica):
            return f"COSD({self.valor.concatenar(tabla,arbol)})"
        return f"COSD({self.valor.traducir(tabla,arbol)})"