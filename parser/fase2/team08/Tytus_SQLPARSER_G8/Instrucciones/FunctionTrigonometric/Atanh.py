...
class Atanh(Instruccion):
    ...
    def traducir(self, tabla, arbol):
        ...
        if isinstance(self.valor, Primitivo):
            return f"ATANH({self.valor.traducir(tabla,arbol).temporalAnterior})"
        elif isinstance(self.valor, Aritmetica):
            return f"ATANH({self.valor.concatenar(tabla,arbol)})"
        return f"ATANH({self.valor.traducir(tabla,arbol)})"