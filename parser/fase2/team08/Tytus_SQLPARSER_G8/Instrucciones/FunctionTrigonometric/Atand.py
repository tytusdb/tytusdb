...
class Atand(Instruccion):
    ...
    def traducir(self, tabla, arbol):
        ...
        if isinstance(self.valor, Primitivo):
            return f"ATAND({self.valor.traducir(tabla,arbol).temporalAnterior})"
        elif isinstance(self.valor, Aritmetica):
            return f"ATAND({self.valor.concatenar(tabla,arbol)})"
        return f"ATAND({self.valor.traducir(tabla,arbol)})"