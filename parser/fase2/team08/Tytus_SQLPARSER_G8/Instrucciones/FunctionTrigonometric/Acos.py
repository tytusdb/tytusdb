...
class Acos(Instruccion):
    ...
    def traducir(self, tabla, arbol):
        ...
        if isinstance(self.valor, Primitivo):
            return f"ACOS({self.valor.traducir(tabla,arbol).temporalAnterior})"
        elif isinstance(self.valor, Aritmetica):
            return f"ACOS({self.valor.concatenar(tabla,arbol)})"
        return f"ACOS({self.valor.traducir(tabla,arbol)})"
