...
class Tanh(Instruccion):
    ...
    def traducir(self, tabla, arbol):
        ...
        if isinstance(self.valor, Primitivo):
            return f"TANH({self.valor.traducir(tabla,arbol).temporalAnterior})"
        elif isinstance(self.valor, Aritmetica):
            return f"TANH({self.valor.concatenar(tabla,arbol)})"
        return f"TANH({self.valor.traducir(tabla,arbol)})"