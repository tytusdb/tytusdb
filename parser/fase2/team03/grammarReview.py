...
def p_statement(t):
    ...
    if len(t) == 3:
        ...
        if token_op.type == 'INTERSECT':
            ...
            punteroinicio(graph_ref)
            addCad("**\<STATEMENT>** ::= \<STM_SELECT> tUnion tAll \<STM_SELECT> ")
            t[0] = Intersect(t[1], t[4], True if t[3] is not None else False, token_op.lineno, token_op.lexpos,
                             graph_ref)
...
def p_statements_begin(t):
    ...
    #Please to not comment the next  5 lines, else after your test uncomment it
    if t[1] is None:
        t[0] = [t[2]]            
    else:
        t[1].append(t[2])
...
def p_return_(t):
    ...
    addCad("**\<RETURN_>** ::=   tReturn  \<LOGICEXPRESSION>  ';'    ")
...
def p_return_opt(t):
    ...
    if len(t) == 4:
        ...
        addCad("**\<RETURN_OPT>** ::=   tReturn  \<LOGICEXPRESSION>  ';'    ")
        t[0] = Return(t[2], t.slice[1].lineno, t.slice[1].lexpos, graph_ref)
        #print(t)
...
def p_stm_drop_function(t):
    ...
    if len(t) == 9:
        ...
        addCad("**\<STM_DROP_FUNCTION** ::= tDrop tFunction [\<IF_EXISTS_OPT>] tIdentifier '(' \<NAME_LIST> ')' \<MODE_DROP_FUNCTION_OPT>  ")
        t[0] = upNodo("token", 0, 0, graph_ref)
    elif len(t) == 5:
        lista = None
        childsProduction  = addNotNoneChild(t,[3])
...
def p_stm_drop_procedure(t):
    ...
    if len(t) == 9:
        ...
        addCad("**\<STM_DROP_PROCEDURE** ::= tDrop tProcedure [\<IF_EXISTS_OPT>] tIdentifier '(' \<LIST_PARAM_FUNCTION_OPT> ')' \<MODE_DROP_FUNCTION_OPT>  ")
        t[0] = upNodo("token", 0, 0, graph_ref)
    elif len(t) == 5:
        lista = None
        childsProduction  = addNotNoneChild(t,[3])
...
def p_stm_create_procedure(t):
    ...
    addCad("**\<STM_CREATE_PROCEDURE>** ::= tCreate tProcedure tIdentifier '(' \<LIST_PARAM_FUNCTION_OPT> ')' tLanguage tPlgsql tAs $ $ \<STM_BEGIN> ';'  $  $   ")
    t[0] = upNodo(True, 0, 0, graph_ref)
...
def p_tab_create_list(t):
    ...
    if len(t) == 7:
        ...
        addCad(
            "**\<TAB_CREATE_LIST>** ::= \<TAB_CREATE_LST> ',' tIdentifier \<TYPE> \<NULLABLE_OPT> \<PRIMARY_KEY_OPT> ")
...
def p_exp_call_function(t):
    ...
    if t[3] != None:
        lista = t[3][0]
        childsProduction.append(lista.graph_ref)
    graph_ref = graph_node(str("stm_call_function"), [t[1],t[2],lista,t[4]],  childsProduction )
