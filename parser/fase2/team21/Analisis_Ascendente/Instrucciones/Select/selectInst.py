                    ...
                    if duplicado == 0:
                        ...
                        for col in Select.columnas:
                            if isinstance(col, Id):
                                ...
                                contadorMath += 1
                        if contadorMath == 1:
                            if contadorFunciones == 0:
                                for col in Select.columnas:
                                    if isinstance(col, Id):
                                        validar = IdSimple(Select, listaTablas, bdactual, contador2, x, listCampos2,
                                                           col, listaAuxiliarC, listaSubquery)
                                        if validar:
                                            consola.append('42725 	ambiguous_function, Existe una ambiguedad')
                                            exceptions.append(
                                                'Error semantico-42725 	ambiguous_function, Existe una ambiguedad')
                                            break
                                        else:
                                            pass
                                        usarListaDeListas = True

                                    elif isinstance(col, IdId):
                                        usarListaDeListas = True
                                        tmpLista = []
                                        for nombreTabla in listaTmp2[0]:
                                            auxTmp = BuscarColumnasLQVP(nombreTabla['Tabla'].id, col, listaTablas,
                                                                        nombreTabla['Tabla'].id)
                                            tmpLista.append(auxTmp)

                                        for nombreTabla in listaTmp2[0]:
                                            auxTmp = BuscarColumnasLQVP4(nombreTabla['Tabla'].id, col, listaTablas,
                                                                         nombreTabla['Tabla'].id)
                                            tmpLista.append(auxTmp)
                                        ...
                                        for nombreTabla in listaTmp[0]:
                                            auxTmp = BuscarColumnasLQVP3(nombreTabla['Tabla'].id, col, listaTablas,
                                                                         nombreTabla['Alias'])
                                            tmpLista.append(auxTmp)
                                            ...
                                            for alias in listaTmp[0]:
                                                if lp != None:
                                                    if (lp['LQBP'] == alias['Alias'] and lp['Columna'] == '*'):
                                                        TablaCompleta(x, bdactual, alias['Tabla'].id, listaTablas,
                                                                      listaSubquery, listaAuxiliarC)
                                                    elif (lp['LQBP'] == alias['Alias']):
                                                        IdIdp(x, col, listCampos2, listaTablas, bdactual,
                                                              alias['Tabla'].id, listaSubquery, listaAuxiliarC)
                                        ...
                                        if isinstance(col.id1, Id):
                                            IdSimple2(Select, listaTablas, bdactual, x, listCampos2, col.id1,
                                                      col.id2.id, listaSubquery, listaAuxiliarC)
                                                ...
                                                for alias in listaTmp[0]:
                                                    if lp != None:
                                                        if (lp['LQBP'] == alias['Alias']):
                                                            ...
                                                            IdIdp2(x, col, listCampos2, listaTablas, bdactual,
                                                                   alias['Tabla'].id, col.id2.id, listaSubquery,
                                                                   listaAuxiliarC)
                                        ...
                                        if col.nombre == "COUNT":
                                            mathNormal(Select, listaTablas, bdactual, contador2, x, listCampos2, col,
                                                       listaAuxiliarC, listaSubquery)
                                        elif col.nombre == "SUM":
                                            sumNormal(Select, listaTablas, bdactual, contador2, x, listCampos2, col,
                                                      listaAuxiliarC, listaSubquery)
                                        elif col.nombre == "AVG":
                                            avgNormal(Select, listaTablas, bdactual, contador2, x, listCampos2, col,
                                                      listaAuxiliarC, listaSubquery)
                                        usarListaDeListas = False
                            else:
                                print("LA ACCION NO SE PUEDE REALIZAR")
                        elif contadorMath == 0:
                            for col in Select.columnas:
                                if isinstance(col, Id):
                                    validar = IdSimple(Select, listaTablas, bdactual, contador2, x, listCampos2, col,
                                                       listaAuxiliarC, listaSubquery)
                                    if validar:
                                        consola.append('42725 	ambiguous_function, Existe una ambiguedad')
                                        exceptions.append(
                                        ...
                                        for alias in listaTmp[0]:
                                            if lp != None:
                                                if (lp['LQBP'] == alias['Alias'] and lp['Columna'] == '*'):
                                                    ...
                                                    IdIdp(x, col, listCampos2, listaTablas, bdactual, alias['Tabla'].id,
                                                          listaSubquery, listaAuxiliarC)
                                    ...
                                    usarListaDeListas = True
                                    if isinstance(col.id1, Id):
                                        IdSimple2(Select, listaTablas, bdactual, x, listCampos2, col.id1, col.id2.id,
                                                  listaSubquery, listaAuxiliarC)
                                    elif isinstance(col.id1, IdId):
                                        # print(col.id1.id1.id)
                                        # print(col.id1.id2.id)
                        ...
                        if usarListaDeListas:
                            print("SIGUE ENTRANDO AQUI")
                            listaCampoOficial.extend(listaDeListas(listaAuxiliarC))
                        else:
                            listaCampoOficial.extend([listaAuxiliarC])
...
def mathNormal(Select,listaTablas,bdactual,contador2,x,listCampos2,col,listaAuxiliarC,sub):
    lista = obtenerCampos(listaTablas, Select, bdactual)
    if lista != None:
        if col.nombre == 'COUNT':
            if isinstance(col.E1,Id):
                if col.E1.id == '*':
                    listaCamposJson = extractTable(bdactual.valor, str(lista[1].id))
                    x.add_column("COUNT(*)", [str(len(listaCamposJson))])
                    sub.append("COUNT(*)")
                    listaAuxiliarC.append(str(len(listaCamposJson)))
    else:
        x.clear()

def sumNormal(Select,listaTablas,bdactual,contador2,x,listCampos2,col,listaAuxiliarC,sub):
    lista = obtenerCampos(listaTablas, Select, bdactual)
    ...
    if lista != None:
        ...
        for campo in lista[0]:  # RECORRO LOS NOMBRES DE LOS CAMPOS DE LA TS
            listCampos2.clear()  # LIMPIO LA LISTA DONDE ALMACENARE LOS DATOS DE CADA COLUMNA
            contador += 1  # INDICA LA POSICION DE LA COLUMNA DONDE OBTENGO LOS VALORES
            if (col.E1.id == lista[0].get(campo).id):
                auxiliarColumna = []
                numero = 0
                for col2 in listaCamposJson:  # RECORRO LOS DATOS DE LA TABLA DE SIMBOLOS
                    if isinstance(col2[contador],(int,float)):
                        numero += col2[contador]
                        #auxiliarColumna.append(col2[contador])
                        #listCampos2.append(str(col2[contador]))
                    else:
                        print("ERROR NO SE PUEDE SUMAR ESTA COLUMNA")
                        break
                numeroAux = str(numero)
                x.add_column("SUM("+lista[0].get(campo).id+")", [numeroAux])  # AGREGO UNA COLUMNA
                listaAuxiliarC.append(numeroAux)
                sub.append(lista[0].get(campo).id)
...
def avgNormal(Select,listaTablas,bdactual,contador2,x,listCampos2,col,listaAuxiliarC,sub):
    lista = obtenerCampos(listaTablas, Select, bdactual)
    ...
    if lista != None:
        ...
        for campo in lista[0]:  # RECORRO LOS NOMBRES DE LOS CAMPOS DE LA TS
            ...
            if (col.E1.id == lista[0].get(campo).id):
                ...
                numeroAux = str(numero/numeroDiv)
                x.add_column("AVG("+lista[0].get(campo).id+")", [numeroAux])  # AGREGO UNA COLUMNA
                listaAuxiliarC.append(numeroAux)
                sub.append(lista[0].get(campo).id)
