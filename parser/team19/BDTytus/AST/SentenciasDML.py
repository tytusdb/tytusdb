...
class Delete(Node.Nodo): 
    ...
    def ejecutar(self, TS, Errores):
        ...
        if self.condicion != '': #falta el where
            self.condicion.ejecutar(TS, Errores)
            col = self.condicion.exp1.val
    ...
    def graficarasc(self, padre, grafica):
        ...
        if self.condicion != '': 
            grafica.node("Condicion%s" % self.mi_id, 'where: %s%s%s' % (str(self.condicion.exp1.val),self.condicion.op,str(self.condicion.exp2.val)))
            grafica.edge(self.mi_id, "Condicion"+self.mi_id)
