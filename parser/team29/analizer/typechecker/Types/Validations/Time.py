...
def typeInterval(val):
    val = val.lower()
    if val =="years" or val =="months" or val =="days" or val =="hours" or val =="minutes" or val =="seconds":
        return True
    elif val =="year" or val =="month" or val =="day" or val =="hour" or val =="minute" or val =="second":
        return True
    return False 
    ...
    for i in range(0,r,2):
        ...
        try:
            if not lst[i].isdigit() or not typeInterval(lst[i+1]):
                error.append( {'Type':"interval",'Descripción':"Parametro desconocido"})
        except:
            #save err
