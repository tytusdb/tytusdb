...
class Pantalla:
    ...
    def inicializarScreen(self):
        ...
        navMenu.add_command(
            label="table de Simbolos", command=self.open_ST
        )
        navMenu.add_command(
        ...
        if consults != None:
            ...
            for consult in consults:
                ...
                if consult != None:
                    ...
                    table = ttk.Treeview(frame,yscrollcommand=table_scroll.set,xscrollcommand=table_scrollX.set,height=12)
    ...
    def parse(self):
        self.refresh()
        input =""
        input = self.txt_entrada.get("1.0", END)  # variable de almacenamiento de la entrada
        result = interpreter.parser(input)
        if len(result['lexical']) + len(result['syntax']) == 0:
            tkinter.messagebox.showerror( title="Mensaje", message="La consulta no contiene errores" )
        else:
    ...
    def open_Reporte(self):  # Abre la pantalla de los reportes de errores
        windowTableS = Pantalla_Error(self.window, self.lexicalErrors, self.syntacticErrors, self.semanticErrors)
