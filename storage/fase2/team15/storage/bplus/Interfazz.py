...
def show_data_bases():
    ...
    database_tree = Storage.serializable.Read('./data/bplus/', 'Databases')
    ...
    canvas_tree.image = PhotoImage(file='./data/bplus/DataBases.png')
...
def show_tables(parent_window, database):
    ...
    db = Storage.serializable.Read(f"./data/bplus/{database}/", database)
    db.graph(database)
    ...
    frame_tree = Frame(canvas_tree)
    canvas_tree.create_window((0, 0), width=10000, height=5000, window=frame_tree, anchor='nw')
    canvas_tree.image = PhotoImage(file=f'./data/bplus/{database}/{database}.png')
...
def extract_table(database, table, parent_window):
    ...
    frame_tree = Frame(canvas_tree)
    canvas_tree.create_window((0, 0), width=10000, height=5000, window=frame_tree, anchor='nw')
    canvas_tree.image = PhotoImage(file=f'./data/bplus/{database}/{table}/{table}.png')
    Label(frame_tree, image=canvas_tree.image).place(x=50, y=50)
...
def getColumn(database, table):
    ...
    dataBaseTree = Storage.serializable.Read('./data/bplus/', "Databases")
    # Get the dbNode
    databaseNode = dataBaseTree.search(dataBaseTree.getRoot(), database)
    # If DB exist
    if databaseNode:
        tablesTree = Storage.serializable.Read(f"./data/bplus/{database}/", database)
        if tablesTree.search(tablesTree.getRoot(), table):
            table_aux = Storage.serializable.Read(f'./data/bplus/{database}/{table}/', table)
...
def table_graph(tupla, key, table, database):
    f = open(f'data/bplus/{database}/{table}/tupla.dot', 'w', encoding='utf-8')
    ...
    f.write('}')
    f.close()
    Storage.os.system(f'dot -Tpng data/bplus/{database}/{table}/tupla.dot -o ./data/bplus/{database}/{table}/tupla.png')
    ...
    tupla_canvas.image = PhotoImage(file=f'./data/bplus/{database}/{table}/tupla.png')
