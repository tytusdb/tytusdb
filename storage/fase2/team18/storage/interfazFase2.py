...
from storage import blockchain as block
import tkinter
from tkinter import *
...
def show_tables(parent_window, database):
    ...
    table_window.title(database + ': tablas')
    ...
    Button(table_window, text='GraphDSD', font='Helvetica 10 bold italic', command=lambda:
    graph_DSD(database, table_window),bg='white', width=15, pady=3).place(x=170, y=15)

def graph_DSD(database, window):
    storage.graphDSD(database)
    table_window = Toplevel(window)
    table_window.geometry('950x610+200+75')
    table_window.title('GraphDSD')
    main_tree = Frame(table_window)
    main_tree.pack(fill=BOTH, expand=1)
    main_tree.configure(background='black')
    ...
    canvas_tree.bind('<Configure>', lambda e: canvas_tree.configure(scrollregion=canvas_tree.bbox('all')))
    frame_tree = Frame(canvas_tree)
    canvas_tree.create_window((0, 0), width=50000, height=10000, window=frame_tree, anchor='nw')
    try:
        canvas_tree.image = PhotoImage(file=f'./{database}.png')
...
def extract_table(database, table, parent_window):
    table_window = Toplevel(parent_window)
    parent_window.iconify()
    has_block = storage.block.blockchain().activo(database, table) 
    ...
    if has_block:
        safe = storage.block.blockchain().fail(database, table)
        if safe:
            canvas_image.image = PhotoImage(file='./storage/Locks/lock_red.png')
            Button(table_window, image=canvas_image.image, command=lambda: safe_mode(database, table, table_window)).place(x=837,y=15)
        else:
            canvas_image.image = PhotoImage(file='./storage/Locks/lock_green.png')
            Button(table_window, image=canvas_image.image, command=lambda: safe_mode(database, table, table_window)).place(x=837,y=15)
        try:
            variable = StringVar(table_window)
            variable.set(datos[0])
            box = OptionMenu(table_window, variable, *datos)
            box.place(x=270, y=15)
        except:
            pass
    Button(table_window, text='GraphDF', font='Helvetica 10 bold italic',command=lambda:
    graph_DF(database, table, table_window),bg='white', width=10, pady=3).place(x=170, y=15)

def graph_DF(database, table, window):
    storage.graphDF(database,table)
    table_window = Toplevel(window)
    table_window.geometry('950x610+200+75')
    table_window.title('GraphDSD')
    main_tree = Frame(table_window)
    ...
    canvas_tree.create_window((0, 0), width=50000, height=10000, window=frame_tree, anchor='nw')
    try:
        canvas_tree.image = PhotoImage(file=f'./'+database+'_'+table+'_DF.png')
        Label(frame_tree, image=canvas_tree.image).place(x=50, y=50)
    except:
        pass
...
def safe_mode(database, table, window):
    try:
        ...
        Label(frame_tree, image=canvas_tree.image).place(x=50, y=50)
        Button(table_window,text='Regresar',command=lambda:no_safe(table_window),font='Helvetica 10 bold italic', bg='red').place(x=25, y=15)
    except:
