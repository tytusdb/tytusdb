...
from utilities.analisys_parser.analizer.typechecker.Types import Type as TYPE
...
def replaceDatabase(name, mode, owner):
    dropDatabase(name)
    avlMode.dropDatabase(name)
    createDatabase(name, mode, owner)
    avlMode.createDatabase(name)
...
def alterColumnsTable(dbName, tName, params):
    ...
    for p in params:
        if p[0] == "ADD":
            ...
            if nCol - nInit == 1:
                avlMode.alterAddColumn(dbName, tName, None)
                nInit += 1
        elif p[0] == "RENAME":
