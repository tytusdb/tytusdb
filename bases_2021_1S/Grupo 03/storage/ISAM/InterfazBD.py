...
from storage.ISAM import Table
...
class PantallaBD():
    def __init__(self, vectorBases):
        ...
        self.titulo = Label(self.contenedor, text="Bases de Datos", font=(
            "Comic Sans MS", 18)).place(x=150, y=20)
        # botones crear eliminar y editar bases de datos
        self.BtnCrearBase = Button(
            self.contenedor, text="Crear Base de Datos", command=self.crear, width=20).place(x=300, y=100)
        self.BtnEliminarBase = Button(self.contenedor, text="Eliminar Base de Datos",
                                      command=self.EliminarBase, width=20).place(x=300, y=160)
        self.BtnEditarBase = Button(self.contenedor, text="Editar Nombre de BD",
                                    command=self.EditarBase, width=20).place(x=300, y=220)
        self.BtnTablas = Button(self.contenedor, text="Ver Tablas",
                                command=self.Tablas, width=20).place(x=300, y=280)
        self.BtnGraficar = Button(self.contenedor, text="Graficar Tablas",
                                  command=self.graficar, width=20).place(x=300, y=340)
    ...
    def EliminarBase(self):
        if len(self.listboxBases.curselection()) != 0:
            ...
            a = messagebox.askquestion(
                "Eliminar", "Quieres eliminar La base de datos \n\t"+elemento)
            if a == "yes":
...
# para visualizar las tablas
    ...
    def Tablas(self):
        if len(self.listboxBases.curselection()) != 0:
            nombreBD = self.listboxBases.get(
                self.listboxBases.curselection()[0])
            self.ventana.destroy()
...
class PantallaCrearBD:
    def __init__(self):
        ...
        self.titulo = Label(self.ventana, text="Ingrese el nombre de la BD", font=(
            "Comic Sans MS", 15)).place(x=20, y=5)
        self.Entrada = Entry(
            self.ventana, textvariable=self.val, width=40).place(x=20, y=40)
        self.Boton = Button(self.ventana, text='CREAR',
                            command=self.llamar(), width=15).place(x=80, y=70)
        self.ventana.mainloop()
...
class PantallaEditarBD:
    def __init__(self, nombreViejo):
        ...
        self.titulo = Label(self.ventana, text="Ingrese el nuevo nombre de la BD", font=(
            "Comic Sans MS", 15)).place(x=15, y=5)

        self.Entrada = Entry(
            self.ventana, textvariable=self.val, width=40).place(x=20, y=40)
        self.Boton = Button(self.ventana, text='EDITAR',
                            command=self.llamar(), width=15).place(x=80, y=70)
        self.ventana.mainloop()
...
class PantallaGraficoTablas:
    def __init__(self, nombreBD):
        ...
        self.scroll = ttk.Scrollbar(
            self.contenedor, orient=VERTICAL, command=self.canvas.yview)
        ...
        self.canvas.create_window(
            (0, 0), window=self.segundocontenedor, anchor="nw")
        imagen = PhotoImage(file="list.png")
        labelimagen = Label(self.segundocontenedor, image=imagen).pack()

        Button(self.segundocontenedor, text="Salir",
               command=self.salir, width=20).pack()
        self.ventana.mainloop()
...
class PantallaTablas:
    def __init__(self, nombreBD):
        ...
        self.contenedor = Frame(self.ventana, width=500, height=400)
        self.contenedor.pack(fill="both", expand=True)
        self.titulo = Label(self.contenedor, text="Tablas de la BD: " +
                            nombreBD, font=("Comic Sans MS", 18)).place(x=110, y=20)
        # boton crear tabla
        Button(self.contenedor, text="Crear Tabla de Datos",
               command=self.crear, width=20).place(x=300, y=80)
        Button(self.contenedor, text="Borrar Tabla",
               command=self.borrar, width=20).place(x=300, y=110)
        Button(self.contenedor, text="Cambiar nombre",
               command=self.renombrar, width=20).place(x=300, y=140)
        Button(self.contenedor, text="Agregar Columna",
               command=self.agregarC, width=20).place(x=300, y=170)
        Button(self.contenedor, text="Eliminar Columna",
               command=self.borrarC, width=20).place(x=300, y=200)
        Button(self.contenedor, text="Agregar PK",
               command=self.agregarPK, width=20).place(x=300, y=230)
        Button(self.contenedor, text="Eliminar PK",
               command=self.eliminarPK, width=20).place(x=300, y=260)
        Button(self.contenedor, text="Ver Tabla",
               command=self.extraerTabla, width=20).place(x=300, y=290)
        Button(self.contenedor, text="Graficar",
               command=self.graficar, width=20).place(x=300, y=320)
        Button(self.contenedor, text="Regresar",
               command=self.salir, width=20).place(x=300, y=350)
    ...
    def crear(self):
        try:
            nombretabla = simpledialog.askstring(
                'Crear Tabla Datos', 'ingrese el nombre de la tabla')
    ...
    def renombrar(self):
        try:
            if len(self.listboxTablas.curselection()) != 0:
                ...
                if a == "yes":
                    print(Storage.alterTable(
                        self.nombreBD, nombreviejo, nombrenuevo))
                    self.listboxTablas.delete(0, END)
                    self.Cargartablas()
        except:
            ""
        ...
        try:
            if len(self.listboxTablas.curselection()) != 0:
                ...
                valor = simpledialog.askstring(
                    'Agregar Columna', 'ingrese el valor por default')
    ...
    def borrarC(self):
        try:
            if len(self.listboxTablas.curselection()) != 0:
                nombretabla = self.listboxTablas.get(
                    self.listboxTablas.curselection()[0])
                columna = simpledialog.askinteger(
                    'Borrar Columna', 'ingrese el numero de columna')
                print(Storage.alterDropColumn(
                    self.nombreBD, nombretabla, columna))
        except:
            ""
        ...
        try:
            if len(self.listboxTablas.curselection()) != 0:
                ...
                entrada = simpledialog.askstring(
                    'Listado de # de columnas', 'ingrese el listado separado por , sin espacios')
                lista = entrada.split(",")
                listafinal = []
                for i in lista:
...
class PantallaTuplas:
    def __init__(self, nombreBD, nombreTabla, listaTuplas):
        ...
        self.contenedor.pack(fill="both", expand=True)
        self.titulo = Label(self.contenedor, text="Tuplas de la tabla: " +
                            self.nombreTabla, font=("Comic Sans MS", 18)).place(x=110, y=10)
        self.titulo = Label(self.contenedor, text="Posee "+str(Storage.rollback('tables/' + self.nombreBD +
                                                                                self.nombreTabla).numberColumns)+" Columnas", font=("Comic Sans MS", 14)).place(x=150, y=40)

        # boton crear tabla
        Button(self.contenedor, text="Extraer Tabla Completa",
               command=self.extraertabla, width=20).place(x=300, y=80)
        Button(self.contenedor, text="Extraer Por Rangos",
               command=self.extraerrango, width=20).place(x=300, y=110)
        Button(self.contenedor, text="Extraer Row (tupla)",
               command=self.extraertupla, width=20).place(x=300, y=140)

        Button(self.contenedor, text="Insertar Registro",
               command=self.insertar, width=20).place(x=300, y=170)

        Button(self.contenedor, text="Actualizar Registro",
               command=self.actualizar, width=20).place(x=300, y=200)

        Button(self.contenedor, text="Eliminar Registro",
               command=self.eliminar, width=20).place(x=300, y=230)
        Button(self.contenedor, text="Eliminar Todo",
               command=self.eliminartodo, width=20).place(x=300, y=260)
        Button(self.contenedor, text="Cargar CSV",
               command=self.cargarCSV, width=20).place(x=300, y=290)

        Button(self.contenedor, text="Regresar",
               command=self.salir, width=20).place(x=300, y=320)
    ...
    def extraerrango(self):
        try:
            columna = simpledialog.askinteger(
                'Rango', 'ingrese el numero de indice de la columna')
            inicio = simpledialog.askstring(
                'Rango', 'ingrese el valor de inicio')
            final = simpledialog.askstring(
                'Rango', 'ingrese el valor del final')
            self.listboxTuplas.delete(0, END)
            self.listaTuplas = Storage.extractRangeTable(
                self.nombreBD, self.nombreTabla, columna, inicio, final)
            self.Cargartuplas()
        except:
            ""
    ...
    def extraertupla(self):
        try:
            ...
            Tupla = Storage.extractRow(
                self.nombreBD, self.nombreTabla, listafinal)
            self.listboxTuplas.delete(0, END)
    ...
    def insertar(self):
        try:
            entrada = simpledialog.askstring(
                'Insertar tuplas', 'ingrese el listado separado por , sin espacios')
            lista = entrada.split(",")
    ...
    def eliminar(self):
        try:
            entrada = simpledialog.askstring(
                'Eliminar tuplas', 'ingrese las pk separado por , sin espacios')
            lista = entrada.split(",")
    ...
    def eliminartodo(self):
        try:
            a = messagebox.askquestion(
                "Eliminar", "Quieres eliminar todos los registros de la tabla \n\t" + self.nombreTabla)
            if a == "yes":
